...
class ResizeRepro(QtWidgets.QMainWindow):
    ...
    def _main_widget(self) -> QtWidgets.QWidget:
        widget = QtWidgets.QWidget(self)
        layout = QtWidgets.QVBoxLayout(widget)
        layout.setContentsMargins(6, 6, 6, 6)
        if self.options.mode == 'blank':
            layout.addWidget(
                QtWidgets.QLabel('Blank QWidget in QMainWindow', widget),
                stretch=1,
            )
    ...
    def _splitter(self) -> QtWidgets.QSplitter:
    ...
    def _control_panel(self) -> QtWidgets.QScrollArea:
        ...
        if self.options.panel == 'flow':
            ...
            layout.addWidget(QtWidgets.QLabel('empty control panel', content))
            layout.addStretch()
        scroll.setWidget(content)
        return scroll

    def _text_area(self) -> QtWidgets.QWidget:
        frame = QtWidgets.QWidget(self)
        layout = QtWidgets.QVBoxLayout(frame)
        label = QtWidgets.QLabel('Text:', frame)
        layout.addWidget(label)
        text = QtWidgets.QTextEdit(frame)
        text.setPlainText('Resize diagonally from the lower-right corner.\n' * 10)
        layout.addWidget(text, stretch=1)
        buttons = QtWidgets.QWidget(frame)
        button_layout = QtWidgets.QHBoxLayout(buttons)
        for name in ('Record', 'Play', 'Loop', 'Randomize time', '?'):
            button_layout.addWidget(QtWidgets.QPushButton(name, buttons))
        layout.addWidget(buttons)
        return frame

    def _note_grid(self) -> QtWidgets.QWidget:
        frame = QtWidgets.QWidget(self)
        layout = QtWidgets.QGridLayout(frame)
        columns = max(1, round(self.options.notes**0.5))
        for i in range(self.options.notes):
            button = QtWidgets.QPushButton(chr(ord('A') + i % 26), frame)
            button.setMinimumSize(50, 30)
            layout.addWidget(button, i // columns, i % columns)
        return frame

    def _button_grid(self) -> QtWidgets.QWidget:
        frame = QtWidgets.QWidget(self)
        layout = QtWidgets.QGridLayout(frame)
        for i in range(max(self.options.notes, 52)):
            layout.addWidget(
                QtWidgets.QPushButton(chr(ord('A') + i % 26), frame), i // 8, i % 8
            )
        return frame


def _control_cell(parent: QtWidgets.QWidget, index: int) -> QtWidgets.QWidget:
    frame = QtWidgets.QWidget(parent)
    layout = QtWidgets.QHBoxLayout(frame)
    layout.setContentsMargins(3, 3, 3, 3)
    layout.addWidget(QtWidgets.QLabel(f'Field {index}', frame))
    if index % 3 == 0:
        ...
        layout.addWidget(QtWidgets.QCheckBox('Enabled', frame))
    return frame
