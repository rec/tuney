...
@pytest.mark.parametrize(
    ...
    [
        (70, 80, 'C\n a', tuney.ui.note_button.MAX_FONT_SIZE),
        (1, 1, 'C\n a', tuney.ui.note_button.MIN_FONT_SIZE),
    ],
)
def test_note_font_size_uses_extremes(
    width: int, height: int, text: str, expected: int
) -> None:
    assert tuney.ui.note_button._note_font_size(width, height, text) == expected


def test_note_font_size_scales_down_for_small_buttons() -> None:
    font_size = tuney.ui.note_button._note_font_size(20, 20, 'C\n a')

    assert (
        tuney.ui.note_button.MIN_FONT_SIZE
        < font_size
        < tuney.ui.note_button.MAX_FONT_SIZE
    )


def test_note_font_size_accounts_for_text_width() -> None:
    assert (
        tuney.ui.note_button._note_font_size(70, 80, 'Very long note\n a')
        < tuney.ui.note_button.MAX_FONT_SIZE
    )


def test_note_button_minimum_size_allows_small_buttons() -> None:
    assert tuney.ui.note_button.MIN_BUTTON_WIDTH == 8
    assert tuney.ui.note_button.MIN_BUTTON_HEIGHT == 30
...
def test_note_button_theme_refresh_preserves_pressed_state() -> None:
    ...
    button = tuney.ui.note_button.NoteButton(
